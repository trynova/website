...
const allPosts = (await Array.fromAsync(posts())).sort(
...
if (import.meta.main) {
  await write(
    ...
    html(renderToString(<Index />), {
      title: "Blog · Nova",
      description: "Blog posts about the Nova project",
      canonical: "/blog/",
    }),
  );
  ...
  allPosts.forEach((post) => {
    ...
    feed.addItem({
      ...
      content: new Marked().parse(post.body, { async: false }) as string,
      author: post.meta.authors.map((author) => ({
        name: author.name,
        link: author.url,
      })),
      date: new Date(post.meta.date.toZonedDateTime("UTC").epochMilliseconds),
    });
  });
